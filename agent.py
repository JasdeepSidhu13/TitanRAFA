...
def _run_offline(state: AgentState) -> None:
    ...
    answer = AnswerabilityEvent(
        refine_round=0,
        caused_by=None,
        in_scope=True,
        speculative=state.speculative,
        reason="offline fixture: in_scope",
    )
    state.append_event(answer)
    ...
    suff = SufficiencyEvent(
        refine_round=0,
        caused_by=plan.event_id,
        passed=True,
        reason="offline fixture: sufficiency passed",
        matched_event_ids=[
            ev.event_id
...
def _run_live_stub(state: AgentState) -> None:
    ...
    answer = AnswerabilityEvent(
        refine_round=0,
        caused_by=None,
        in_scope=state.question_type != "out_of_scope",
        speculative=state.speculative or state.question_type == "speculative",
        reason="live stub: not yet implemented",
    )
    state.append_event(answer)
    ...
    if not answer.in_scope:
        state.write_run_complete(
            ...
            refusal_reason="question_type=out_of_scope",
        )
        return
